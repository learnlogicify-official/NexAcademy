...
import { ProblemSolvingWrapper, triggerStreakModal } from "@/components/problem-solving-wrapper"
...
export default function ProblemSolvingPage() {
  ...
  const handleSubmit = () => {
    ...
    setTimeout(() => {
      if (problem) {
        ...
        // Trigger streak modal with a robust approach to ensure it shows
        console.log("[ProblemPage] About to trigger streak modal")
        
        // First attempt immediately
        const success = triggerStreakModal({
          streakEstablished: true,
          currentStreak: 3,
          highestStreak: 7
        })
        
        // If the first attempt fails, retry with increasing delays
        if (!success) {
          console.log("[ProblemPage] First attempt failed, scheduling retries")
          
          // Try again after 500ms, 1s, and 2s to ensure it gets through
          setTimeout(() => {
            console.log("[ProblemPage] Retry 1")
            triggerStreakModal({
              streakEstablished: true,
              currentStreak: 3,
              highestStreak: 7
          ...
          setTimeout(() => {
            console.log("[ProblemPage] Retry 2")
            triggerStreakModal({
              streakEstablished: true,
              currentStreak: 3,
              highestStreak: 7
  ...
  return (
    <ProblemSolvingWrapper>
      {/* Debug info */}
      <div className="fixed bottom-2 right-2 z-50 bg-black bg-opacity-70 text-white p-2 rounded text-xs">
        {/* Hidden in production, visible in development for debugging */}
        {process.env.NODE_ENV === 'development' && (
          <button 
            onClick={() => {
              console.log("Manual streak trigger")
              triggerStreakModal({
                streakEstablished: true,
                currentStreak: 5,
                highestStreak: 10
