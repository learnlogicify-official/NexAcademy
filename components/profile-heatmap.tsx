...
export function ProfileHeatmap({ data, submissionStats }: ProfileHeatmapProps) {
  const [activityType, setActivityType] = useState("all")
  const [displayMode, setDisplayMode] = useState<"activity" | "verdict">("verdict")
  ...
  const getDominantVerdict = (date: Date) => {
    ...
    return activity.verdictSummary.reduce((prev, current) => 
  ...
  // Get color based on verdict for verdict mode
  const getVerdictColor = (date: Date) => {
    const dominantVerdict = getDominantVerdict(date);
    
    if (!dominantVerdict) {
      return "bg-gray-200 dark:bg-[#2C2C2C]";
    }

    // Convert hex to tailwind classes
    switch(dominantVerdict.status) {
      case 'ACCEPTED':
  ...
  // Get verdict summary for a specific date
    ...
    <div className="w-full bg-white dark:bg-[#121212] p-4 rounded-lg flex flex-col">
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          <div className="bg-gray-100 dark:bg-gray-800 rounded-md flex text-xs">
            <button 
              className={`px-3 py-1 rounded-md ${displayMode === 'activity' ? 'bg-primary text-white' : ''}`}
              onClick={() => setDisplayMode('activity')}
            >
              Activity
            </button>
            <button 
              className={`px-3 py-1 rounded-md ${displayMode === 'verdict' ? 'bg-primary text-white' : ''}`}
              onClick={() => setDisplayMode('verdict')}
            >
              Verdicts
            </button>
          </div>
        ...
        <div className="flex flex-wrap gap-2 mb-4">
          {submissionStats?.map((stat, i) => (
            ...
            >
              ...
              <span className="font-medium" style={{ color: stat.color }}>{stat.readableStatus}</span>
              <span>{stat.count}</span>
              <span className="text-gray-400">
                ({Math.round((stat.count / verdictTotals.total) * 100)}%)
              </span>
            </div>
          ))}
        ...
        {displayMode === 'activity' ? (
          <div className="flex items-center justify-between">
            ...
            <div className="text-xs text-gray-600 dark:text-gray-300">
              <span className="mr-4">Active days: <strong>{activeDays}</strong> ({activityPercentage}%)</span>
              <span className="mr-4">Total submissions: <strong>{totalActivities}</strong></span>
              <span>Daily average: <strong>{(totalActivities / Math.max(1, activeDays)).toFixed(1)}</strong></span>
            </div>
          </div>
        ) : (
